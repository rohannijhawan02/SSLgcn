...
def get_gcn_prediction(smiles: str, endpoint: str) -> Dict[str, Any]:
    ...
    Requires a trained model for the requested endpoint.
    ...
    """
    if not GCN_AVAILABLE or gcn_loader is None:
        raise HTTPException(status_code=503, detail="GCN inference is unavailable; install dependencies and provide checkpoints")
    ...
    try:
        ...
        if pred_result is None:
            raise HTTPException(status_code=503, detail=f"No trained GCN checkpoint is available for {endpoint}")
        return {
            "model": "GCN",
            "endpoint": endpoint,
            "endpoint_name": endpoint_info["name"],
        ...
        raise HTTPException(status_code=503, detail=f"GCN inference failed: {e}") from e
