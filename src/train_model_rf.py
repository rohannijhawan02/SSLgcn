"""
Random Forest (RF) Model Training for Toxicity Prediction
Uses ECFP4 fingerprints for molecular encoding

Based on: "Random Forest (RF) was tested. The compounds were encoded using 
...
class RFTrainer(BaselineModelTrainer):
    """
    Specialized trainer for Random Forest classifier.
    """
    
    def __init__(self, toxicity_name, **kwargs):
        super().__init__(toxicity_name, **kwargs)
        
        # RF-specific configuration
        self.model_name = 'RF'
        self.param_grid = {
        ...
        print(f"\n{'='*80}")
        print(f"Random Forest (RF) Trainer Initialized")
        print(f"{'='*80}")
    ...
    def train_rf(self):
        ...
        """
        print(f"\n{'-'*80}")
        print(f"Training Random Forest for {self.toxicity_name}")
        print(f"{'-'*80}")
        ...
        self.save_model('RF', results)
        self.save_results('RF', results)
        
        return results


def train_rf_all_toxicities():
    """
    Train Random Forest for all 12 toxicity endpoints.
    """
    toxicity_endpoints = [
    ...
    print("\n" + "="*80)
    print("TRAINING RANDOM FOREST FOR ALL TOXICITY ENDPOINTS")
    print("="*80)
    ...
    if all_results:
        ...
        summary_path = 'results/baseline_models/RF_all_toxicities_summary.csv'
        summary_df.to_csv(summary_path, index=False)
        
        print("\n" + "="*80)
        print("RANDOM FOREST TRAINING COMPLETE - SUMMARY")
        print("="*80)
...
def main():
    """
    Main function - train RF for single or all toxicity endpoints.
    """
    import argparse
