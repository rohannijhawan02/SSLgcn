"""
K-Nearest Neighbor (KNN) Model Training for Toxicity Prediction
Uses ECFP4 fingerprints for molecular encoding

Based on: "K-Nearest Neighbor (KNN) was tested. The compounds were encoded 
...
class KNNTrainer(BaselineModelTrainer):
    """
    Specialized trainer for K-Nearest Neighbor (KNN) classifier.
    """
    ...
    def __init__(self, toxicity_name, **kwargs):
        ...
        print(f"\n{'='*80}")
        print(f"K-Nearest Neighbor (KNN) Trainer Initialized")
        print(f"{'='*80}")
    ...
    def train_knn(self):
        """
        Train KNN model with hyperparameter optimization.
        
        Returns:
            dict: Trained model and results
        """
        print(f"\n{'-'*80}")
        print(f"Training KNN for {self.toxicity_name}")
        print(f"{'-'*80}")
        ...
        self.save_model('KNN', results)
        self.save_results('KNN', results)
        
        return results


def train_knn_all_toxicities():
    """
    Train KNN for all 12 toxicity endpoints.
    """
    toxicity_endpoints = [
    ...
    print("\n" + "="*80)
    print("TRAINING KNN FOR ALL TOXICITY ENDPOINTS")
    print("="*80)
    ...
    if all_results:
        ...
        summary_path = 'results/baseline_models/KNN_all_toxicities_summary.csv'
        summary_df.to_csv(summary_path, index=False)
        
        print("\n" + "="*80)
        print("KNN TRAINING COMPLETE - SUMMARY")
        print("="*80)
...
def main():
    """
    Main function - train KNN for single or all toxicity endpoints.
    """
    import argparse
